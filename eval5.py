...
class KonaneAI:
    ...
    def update_state(self, move):
        ...
        if move in self.state.successors:
            self.state = self.state.successors.get(move)
            return
    ...
    def evaluation(self, depth, state):
        ...
        centre_eval = (centre_control - centre_control_opp) * 0.01
        total_moves = (total_moves - total_moves_opp) * 0.01
        evaluation = agent_can_move - opp_can_move + total_moves + centre_eval
        return evaluation
    ...
    def generate_valid_moves(self, state):
        ...
        moves = [] # list of valid moves
        total_pieces = set() #get positions of pieces it can move
            ...
            for x in range(8):
                ...
                if state.board.get_board()[y][x] == state.colours_turn:
                    ...
                    for l in range(y+1, 8, 2):
                        ...
                        total_pieces.add(initial)
                        moves.append(f"{initial}-{final}") 
        state.pieces_can_move = len(total_pieces)
        return moves
